...
We also have an easy way of accessing the "length" of a vector now, by declaring
that the square of the length of a vector is the inner product of a vector with
itself, i.e. $\lVert v \rVert ^2 = \langle v, v \rangle$. We also call this
length the norm of a vector.
...
Alternatively and equivalently, we can say that a basis $b_0, \ldots, b_n$
generates an inner product if and only if that inner product maps $\langle b_i,
b_i \rangle = 1$ and $\langle b_i, b_j \rangle = 0$

That is a basis generates an inner product if it "thinks" that those basis
vectors are orthonormal (that is the basis vectors are orthogonal to each other
and every basis vector's norm is 1).

We use the word "generate" here because we can think of this process as
using a basis as our "ruler" and "compass" where we declare by fiat that all our
basis vectors have length $1$ and are perpendicular to each other. Then we use
that to extrapolate out to all other vectors. This is in much the same way that
the meter was at one time defined by declaring by fiat that a particular bar was
exactly $1$ meter and then measuring all other lengths relative to that bar. So
we could say that that bar "generated" our length measurement for all other
lengths.

To expand upon "extrapolating out to all other vectors", having defined what the
inner product is by fiat on our basis vectors, we induce an inner product on all
other vectors by declaring that our inner product is bilinear and calculating
the inner product for all other pairs of vectors by splitting the vectors into
linear combinations of basis vectors using the basis we chose to generate the
inner product.

So for example, if I want to use the basis $(1, 2)$ and $(1, 0)$ to generate an
inner product for $R^2$, I first declare that 

```math
\langle (1, 2), (1, 0) \rangle &= 0 \\
\langle (1, 0), (1, 2) \rangle &= 0 \\
\langle (1, 2), (1, 2) \rangle &= 1 \\
\langle (1, 0), (1, 0) \rangle &= 1
```

Then for any arbitrary set of vectors, say $(2, 2)$ and $(1, 4)$, I can
calculate the inner product in the following manner (using (bi)linearity to
distribute across sums and scalar multiplication):

```math
\langle (2, 2), (1, 4) \rangle &= \langle 1 \cdot (1, 2) + 1 \cdot (1, 0), 2 \cdot (1, 2) + (-1) \cdot (1, 0)  \rangle \\
&= \langle 1 \cdot (1, 2), 2 \cdot (1, 2) + (-1) \cdot (1, 0) \rangle + \langle 1 \cdot (1, 0), 2 \cdot (1, 2) + (-1) \cdot (1, 0)  \rangle
&= \langle 1 \cdot (1, 2), 2 \cdot (1, 2) \rangle + \langle 1 \cdot (1, 2),(-1) \cdot (1, 0) \rangle + \langle 1 \cdot (1, 0) + (-1) \cdot (1, 0)  \rangle + \langle 2 \cdot (1, 2) + (-1) \cdot (1, 0)  \rangle
&= 1 \cdot 2 \cdot \langle (1, 2), (1, 2) \rangle + 1 \cdot (-1) \cdot \langle
(1, 2), (1, 0) \rangle + 1 \cdot (-1) \cdot \langle (1, 0), (1, 0) \rangle + 2
\cdot (-1) \cdot \langle (1, 2), (1, 0) \rangle
&= 1 \cdot 2 \cdot 1 + 1 \cdot (-1) \cdot 0 + 1 \cdot (-1) \cdot 1 + 2 \cdot (-1) \cdot 0
&= 1 \cdot 2 + 1 \cdot (-1)
&= 1
```

You might notice that given $(2, 2) = 1 \cdot (1, 2) + 1 \cdot (1, 0)$ and $(1,
4) = 2 \cdot (1, 2) + (-1) \cdot (1, 0)$ we've just ended summing a bunch of
pairwise multiplications, which looks suspiciously like the dot produt.

Indeed, For any inner product generated by a basis $B = b_0, \ldots, b_n$, the
inner product of two arbitrary vectors $v$ and $w$ is the sum of the pair-wise
multiplication of scalar constants of $v$ and $w$. That is, given $v = v_0 b_0 +
\cdots + v_n b_n$ and $w = w_0 b_0 + \cdots + w_n b_n$, their inner product is
$\sum _{0 \le i \le n} v_i w_i$

And hence we've motivated why the dot product is so important! Given the
appropriate basis, any inner product is just the dot product.
...
E.g. the transpose of the matrix 
...
Let's return to the definition of a matrix, but now we're going to define them
in a different way.

As a reminder, we thought of a matrix as a lookup table where we took each basis
vector in our domain, looked up the relevant column, and then mapped it to each
column, where "mapping it to a column" means mapping the corresponding basis
vector to a linear combination of basis vectors where each scalar coefficient is
an entry in the column of the matrix.
...
maps $(1, 0) \mapsto 1 \cdot (1, 0, 0) + 2 \cdot (0, 1, 0) + 3 \cdot (0, 0, 1)$ and $(0, 1) \mapsto 4 \cdot (1, 0, 0) + 5 \cdot (0, 1, 0) +6 \cdot (0, 0, 1)$.

Another way we could use the columns of this lookup table is instead as
"feature detectors." 

What do I mean by "feature detectors?" Well let's imagine I had two vectors $(1,
1, 0)$ and $(0, 1, 1)$ that I wanted to use as "detectors" where for any
arbitrary vector $(a, b, c)$, I'd create a comparison score that compares $(a,
b, c)$ to $(1, 1, 0)$ and $(0, 1, 1)$, resuting in a final 2-tuple score.

Each of my two vectors $(1, 1, 0)$ and $(0, 1, 1)$ functioned as a "feature" for
which we're trying

For example, if we had a weather description problem where we were given three
data points every day: the average temperature (Celsius), the average moisture
content in the air (mg of water per cubic meter of air), and the amount of
precipitation (mm of equivalent rainfall), together making up an $R^3$ space,
you could imagine that we might have a "feature detector" vector that detects
snow, which is the vector $()$

We could also have a "mugginess" "feature detector" vector which might be $(0.2,
0.6, 0)$. By using the dot product, this would result in a high "mugginess" score
for a day that was 35 degrees Celsius, had 30 mg of water per cubic meter of air,
and had 10 mm of rain. In particular our "feature detector" here would give a
score of $0.2 \cdot 30 + 0.6 \cdot 35 + 0 \cdot 10 = 27$.

More generally, we could say that this particular "feature detector" "activates"
highly (i.e. high inner product) on days with both high temperature and high
moisture content in the air, moderately if only one of those two conditions
holds, and not at all or even negatively "activates" with low temperature and
low moisture content. Note that our "mugginess" "feature detector" is completely
insensitive to the amount of rainfall.

We could also have another "feature detector", which might be a "wetness feeling"
detector. That we could say is the vector $(0, 0.2, 1)$. This detector activates
strongly when we have both moisture content in the air and rainfall, but is
completely insensitive to temperature.

Another way of saying this is that we have created two directions/vectors in
$R^3$, a "mugginess direction" in and a "wetness feeling direction."

We can put those together in a single lookup table again that looks like

```math
\begin{bmatrix} 0.2 & 0 \\\ 0.6 & 0.2 \\\ 0 & 1\end{bmatrix}
```

This would allow us to calculate a combined mugginess and wetness-feeling score.

Another way of saying this is that we have created a function that goes from
the three-dimensional space of temperature, moisture content, and rainfall to
the two-dimensional space of mugginess and "wetness feeling" where we use our
two "feature detectors" to generate a new pair in our two-dimensional space by
seeing how strongly each of them activates given an arbitrary point in our
original three-dimensional space.

*Exercise*:

> Given the point in temperature, moisture content, and rainfall space $(25,
> 0.1, 5.2)$ calculate the equivalent point in mugginess, wetness-feeling space
> using the above lookup table.
...
Can we express this via matrix multiplication? Yes we can! It turns out that
this interpretation of a $3 \times 2$ matrix is exactly the same as multiplying
the transpose of that matrix by a vector of length $2$.

*Exercise*:

> Once again, given the point in temperature, moisture content, and rainfall space $(25,
> 0.1, 5.2)$ calculate the equivalent point in mugginess, wetness-feeling space
> using the above lookup table. This time though, treat it as a $3 \times 2$
> matrix and first take its transpose before multiplying by the vector $(25,
> 0.1, 5.2)$ using standard matrix multiplication.
...
Let's examine the first column of our matrix (i.e. the "mugginess" column) in
more detail. The "forward direction" of the matrix 

```math
\begin{bmatrix} 0.2 & 0 \\\ 0.6 & 0.2 \\\ 0 & 1\end{bmatrix}
```

takes "one unit" of mugginess and translates it into units of temperature,
moisture content, and rainfall ($0.2$ degrees, $0.6$ mg, and $0$ mm
respectively).

The "transpose direction" of that matrix treats the vector $(0.2, 0.6, 0)$ as a
mugginess "detector" instead and computes a mugginess score of any day via the inner
product of any 3-vector $(a, b, c)$ with $(0.2, 0.6, 0)$.

Let's drive home this intuition with some diagrams.

If we specialize ourselves to matrices of functions from $R^n \to R^2$ all using the
standard basis of $R^n$, we can draw a 2-d diagram corresponding to a given
matrix by writing out the corresponding vectors that our matrix (i.e. our linear
function) maps the basis vectors (i.e. $(1, 0)$ and $(0, 1)$) to.

Let's call this a "stick diagram" (this isn't standardized terminology so I'll
continue referring to it in quotes).
...
corresponds to this "stick diagram."

*Exercise*:

Set the stick diagram
